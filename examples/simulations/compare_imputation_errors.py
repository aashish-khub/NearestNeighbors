...
if os.path.exists(data_file):
    ...
    model_used = df['latent_factor_combination_model'].iloc[0]  # Assuming it's consistent across rows
    ...
    summary_stats = df.groupby(["size", "estimation_method", "miss_prob", "snr"])['est_errors_squared'].describe()
    logger.info(summary_stats)
    # Reset index to add size and estimation_method as columns
    summary_stats = summary_stats.reset_index()
    summary_stats_filename = f"summary_stats_model{model_used}_missprob{miss_prob}_snr{snr}.csv"
    ...
    for idx, method in enumerate(summary_stats['estimation_method'].unique()):
        ...
        slope, intercept, r_value, p_value, std_err = linregress(subset['size'], np.log(subset['mean']))
        label_with_slope = f"{method} (n^{{{slope:.3f}}})"
        color_code = sns.color_palette("husl", len(summary_stats['estimation_method'].unique()))[idx]
        # Plot dots
        plt.scatter(
            subset['size'],
            subset['mean'],
    ...
    y_ticks = np.logspace(np.floor(np.log10(min(ymins))).astype(int), 
                          np.ceil(np.log10(max(ymaxs))).astype(int), 
                          num=10)  # Adjust 'num' for more or fewer ticks
    plt.yticks(y_ticks, [f"{tick:.1e}" for tick in y_ticks])

    plt.title(f"Mean Imputation Error with Regression Lines\n(model={model_used}, miss_prob={miss_prob}, snr={snr})")
    plt.xlabel("Matrix Size (2^n)")
    ...
    plt.tight_layout()
    # Save the plot with model description in the filename
    regression_plot_filename = f"mean_error_with_regression_model{model_used}_missprob{miss_prob}_snr{snr}.png"
    plt.savefig(os.path.join(output_dir, regression_plot_filename), dpi=300)
    plt.close()
    logger.info(f"Generated mean error plot with regression lines: {regression_plot_filename}")
else:
