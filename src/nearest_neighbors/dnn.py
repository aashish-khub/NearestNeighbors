...
class DistNNEstimator(ABC):
    ...
    @abstractmethod
    def average_distributions(self, distributions: list[npt.NDArray[np.float64]]) -> npt.NDArray[np.float64]:
    ...
    def predict(self, row_index: int, column_index: int, data: list[list[npt.NDArray[np.float64]]]) -> npt.NDArray[np.float64]:
        ...
        if self.nn_type == 'user-user':
            ...
            distance_cells = [(row_index, j) for j in range(num_columns) if j != column_index]
            ...
            distance_cells = [(i, column_index) for i in range(num_rows) if i != row_index]
