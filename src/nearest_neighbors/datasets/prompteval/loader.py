...
class PromptEvalDataLoader(NNDataLoader):
    ...
    def download_data(self) -> None:
        ...
        pe_data = load_dataset("PromptEval/PromptEval_MMLU_Correctness", 'abstract_algebra')
        pe_data = cast(Dataset, pe_data)
        pe_data.to_csv(os.path.join(self.save_dir, "prompteval_correctness_data.csv"), index=False)
    ...
    def process_data_scalar(self, cached: bool = False, agg: str = "mean") -> tuple[np.ndarray, np.ndarray]:
        ...
        if not cached:
            ...
            df_final = pd.read_csv(os.path.join(self.save_dir, "prompteval_correctness_data.csv"))
        ...
        if self.save_processed:
            np.save(os.path.join(self.save_dir, "prompteval_correctness_data.npy"), data)
            np.save(os.path.join(self.save_dir, "prompteval_correctness_mask.npy"), mask)
    ...
    def process_data_distribution(self, cached: bool = False) -> tuple[np.ndarray, np.ndarray]:
        ...
        self.data = data2d
        if self.save_processed:
            np.save(os.path.join(self.save_dir, "prompteval_correctness_data_distribution.npy"), data2d, allow_pickle=True)
            np.save(os.path.join(self.save_dir, "prompteval_correctness_mask_distribution.npy"), self.mask, allow_pickle=True)
        return self.data, self.mask
    ...
    def _form_data(self, df_final: pd.DataFrame) -> np.ndarray:
        ...
        for id_model, model in enumerate(models):
            for id_format, format in enumerate(formats):
                df_sub = df_final[(df_final['format'] == format) & (df_final['model'] == model)]
                task_examples = []
                for id_task, task in enumerate(tasks):
                    df_sub_task = df_sub[df_sub['task'] == task]
                    task_examples = df_sub_task["correctness"].to_list()
    ...
    def _load_config_data(self, config_name:str) -> pd.DataFrame | None:
        ...
        try:
            ...
            dataset = load_dataset("PromptEval/PromptEval_MMLU_correctness", config_name)        
            list_dfs = []
            for key in dataset:
