...
class EarningsDataLoader(NNDataLoader):
    ...
    def __init__(self, 
        ...
        self.cols = [(year, quarter) for year in range(2010, 2025) for quarter in range(1, 5)]
        ...
        if self.agg == "mean":
            ...
            raise ValueError(f"Aggregation method {self.agg} not supported. Supported methods: {self.supported_aggs}")
    ...
    def process_data_distribution(self) -> tuple[npt.NDArray, npt.NDArray]:
        ...
        for i, oftic in enumerate(self.rows):
            for j, (year, quarter) in enumerate(self.cols):
                data = self.quarterly_data.get((oftic, year, quarter))
                if data is not None:
                    data_matrix[i, j] = empirical_quantile_function(data['value'].to_numpy())
                    mask_matrix[i, j] = 1
                else:
                    data_matrix[i, j] = np.nan
                    mask_matrix[i, j] = 0
