...
class StarNNEstimator:
    ...
    def impute(
        ...
        # print(f"observed_rows: {observed_rows} and n_observed: {len(observed_rows)}")
        ...
        row_distances_vector = row_distances[row, observed_rows] - 2 * noise_variance
        row_dist_min = min(0, np.min(row_distances_vector))
        row_distances_vector -= row_dist_min
        if mask_array[row, column]:
            row_distances_vector = np.where(
                observed_rows == row, 0, row_distances_vector
            )

        mean_distance = np.mean(row_distances_vector)
        dist_diff = row_distances_vector - mean_distance
        # print(f"row_dist_vector: {row_dist_vector}")
        # print(f"row_dist_vector + 2*noise_variance: {row_dist_vector + 2*noise_variance}")
        # print(f"delta: {delta} and log(1/delta): {np.log(1/delta)}")

        weights = (1 / n_observed) - dist_diff / (
            8 * noise_variance * np.log(1 / delta)
        )
        # print(f"normalized distance difference: {dist_diff/(8 * noise_variance * np.log(1/delta))}")
...
class star_nn:
    ...
    def __str__(self):
        return "StarNNImputer(noise_variance={}, delta={})".format(
            self.noise_variance, self.delta
        )
    ...
    def set_row_distances(
        ...
        if distance_matrix is not None:
                ...
                for j in range(i + 1, n_rows):
                    ...
                    for k in range(n_cols):
                        if not overlap_cols[k]:
                            continue
                        self.row_distances[i, j] += self.data_type.distance(
            ...
            raise ValueError(
                "Either distance_matrix or both data_array and mask_array must be provided"
            )
    ...
    def impute_matrix(self, data_array: npt.NDArray, mask_array: npt.NDArray) -> float:
        ...
        for i in range(n_rows):
            for j in range(n_cols):
                imputed_data[i, j] = self.estimation_method.impute(
                    i,
                    j,
                    data_array,
                    mask_array,
    ...
    def fit(
        ...
        for iteration in range(max_iterations):
            print(
                f"Iteration {iteration + 1}/{max_iterations} with noise variance: {self.noise_variance}"
            )
            ...
            if len(diff) > 0:
                ...
                if (
                    ...
                    print(
                        f"Converged after {iteration + 1} iterations and final noise variance: {new_variance_estimate}"
                    )
